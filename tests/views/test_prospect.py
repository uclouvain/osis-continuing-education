...
class ProspectListTestCase(TestCase):
    ...
    def test_error_message_no_draft_prospectt_selected(self):
        response = self.client.post(
            reverse('prospect_delete'),
            data={},
            follow=True,
        ...
        self.assertEqual(msg[0], _("Please select at least one prospect"))

    def test_draft_deleted(self):
        prospect = ProspectFactory(
        )
        self.assertTrue(Prospect.objects.filter(id=prospect.id).exists())
        response = self.client.post(
            reverse('prospect_delete'),
            data={'selected_action': prospect.id},
        )
        self.assertFalse(Prospect.objects.filter(id=prospect.id).exists())
