...
class ViewStudentRegistrationTestCase(TestCase):
    ...
    def test_student_registration_detail(self):
        url = reverse('student_registration_detail', args=[self.admission_accepted.id])
        response = self.client.get(url)
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'student/registration_detail.html')

    def test_registration_detail_not_found(self):
        response = self.client.get(reverse('student_registration_detail', kwargs={
            'admission_id': 0,
        }))
        self.assertEqual(response.status_code, 404)

    def test_registration_edit_not_found(self):
        response = self.client.get(reverse('student_registration_edit', kwargs={
            'admission_id': 0,
        }))
        self.assertEqual(response.status_code, 404)

    def test_edit_get_registration_found(self):
        url = reverse('student_registration_edit', args=[self.admission_accepted.id])
        response = self.client.get(url)
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'student/registration_form.html')
    ...
    def test_edit_post_registration_found(self):
        ...
        self.assertRedirects(response, reverse('student_registration_detail', args=[self.admission_accepted.id]))
